...
│
├── NO → Does it follow fixed rules written by a human?
│         │
│         ├── YES → Traditional Programming (NOT AI)
│         │         Examples: Calculator, alarm clock, basic website
│         │
│         └── NO → Just a tool (not even a program)
│                   Examples: Light switch, hammer, bicycle
│
└── YES → Does it improve or adapt over time?
          │
          ├── YES → Machine Learning / AI ✅
          │         Examples: Instagram recommendations, Gmail spam filter,
          │         Google Maps, Netflix, ChatGPT
          │
          └── NO → Might be simple pattern matching (basic/borderline)
                    Examples: Basic keyword spam filter (just checks for
                    specific words, never updates its list)
```
...
| 5 | **Roomba vacuum** | Some — basic navigation, learns room layout | YES — physical vacuum that moves around | Simple AI + Robot |
